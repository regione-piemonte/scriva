...
export class ErrorValidatorService {
  ...
  public minimumAgeValidator(minAge: number): ValidatorFn {
    return (control: AbstractControl): { [key: string]: any } | null => {
      // if control value is not null and is a number
      if (control.value !== null) {
        return moment().diff(control.value, 'years') < minAge
          ? { minimumAge: true }
          : null;
      }
      return null;
   ...
   * @returns ValidationErrors
   */
  public stringMustMatchCaseInsensitive(
    controlName: string,
    ...
    return (formGroup: FormGroup) => {
      ...
      if (
        ...
        (<string>matchingControl.value).toLowerCase()
      ) {
  ...
  public masterSlaveValue(
    masterControlName: string,
    slaveControlName: string
  ): (f: FormGroup) => void {
    return (formGroup: FormGroup) => {
  ...
  /**
   * Aggiunge e rimuove le classi di validazione sugli ion-item della pagina
   * @param  {any} element
   * @param  {AbstractControl} control
   * @returns void
   */
  private _setControlCss(element: any, control: AbstractControl): void {
    if (control.touched) {
      this._renderer.addClass(element, 'ng-touched');
      this._renderer.removeClass(element, 'ng-untouched');
    } else {
      this._renderer.addClass(element, 'ng-untouched');
      this._renderer.removeClass(element, 'ng-touched');
    }

    if (control.dirty) {
      this._renderer.addClass(element, 'ng-dirty');
      this._renderer.removeClass(element, 'ng-pristine');
    } else {
      this._renderer.addClass(element, 'ng-pristine');
      this._renderer.removeClass(element, 'ng-dirty');
    }

    if (control.enabled) {
      if (control.valid) {
        this._renderer.addClass(element, 'ng-valid');
        this._renderer.removeClass(element, 'ng-invalid');
      } else {
        this._renderer.addClass(element, 'ng-invalid');
        this._renderer.removeClass(element, 'ng-valid');
      }
    } else {
      this._renderer.addClass(element, 'ng-valid');
      this._renderer.removeClass(element, 'ng-invalid');
