...
import { DichiarazioniOrientamentoVERConsts } from './utilities/dichiarazioni-ver.consts';
import {
  IDatiACRegionePiemonteVER,
  IDatiACSpecificaVER,
  IDichiarazioneACPubVERSpecifica,
  ILinkAcWebVERReq,
  ILinkAcWebVERRes,
} from './utilities/dichiarazioni-ver.interfaces';
...
import { IConfigsSaveQuadro, RequestSaveBodyQuadro } from 'src/app/shared/components/stepper-istanza/utilities/stepper-istanza.interfaces';

@Component({
  selector: 'app-dichiarazioni-ver',
  templateUrl: 'dichiarazioni-ver.component.html',
  styleUrls: ['dichiarazioni-ver.component.scss'],
})
export class DichiarazioniVerComponent
  extends StepperIstanzaComponent
  implements OnInit
{
  /** DichiarazioniVERConsts con le costanti del componente. */
  DVER_C = new DichiarazioniOrientamentoVERConsts();
   ...
   * @returns Observable<ILinkAcWebVERRes> con le informazioni scaricate.
   */
  private getAutoritaCompetentiIstanza$(
    istanza: Istanza
  ): Observable<ILinkAcWebVERRes> {
  ...
  private scaricaLinkAcWebIstanza$(
    ...
    const linkAcWebReq: ILinkAcWebVERReq = {
  ...
  private getAcWebPubIstanzaVIA$(
    ...
    const codAdempimento: string = codTipoAdempimento;
    const info: TipoInfoAdempimento = this.DVER_C.INFO_VIA_DICH_PUB;
    const chiave: string =
  ...
  private disabilitaInput() {
    ...
    //this.formDichiarazioni.get('dchr_spec_cond_amb_screening').disable();
  }
  ...
  private getCaricaQdrDichiarazioni(q: any) {
    ...
    //JIRA-1660
    if (this.adempimento.id_adempimento === 1) {
      if (
        q.dchr_no_nuova_real_aree_siti &&
        q.dchr_no_nuova_real_aree_siti.check
      ) {
        this.formDichiarazioni
          .get('dchr_no_nuova_real_aree_siti')
          .setValue(q.dchr_no_nuova_real_aree_siti.check);
        this.formDichiarazioni.get('dchr_spec_cond_amb_screening').enable();
        if (
          q.dchr_spec_cond_amb_screening &&
          q.dchr_spec_cond_amb_screening.radio
        ) {
          this.formDichiarazioni
            .get('dchr_spec_cond_amb_screening')
            .setValue(q.dchr_spec_cond_amb_screening.radio);
        }
      }
    }
  ...
  private getCategoriaAllegati(qdrDichiarazioni: any) {
    ...
    // resetto i flag relativi alla presenza degli allegati (DOC-AMM-FAC-NOPUBB, SEGR-IND-COMM)
    this.doc_amm_nopubb = false;
    this.doc_segr_ind_com = false;
  ...
  private buildFormDichiarazioni() {
    this.formDichiarazioni = this.fb.group({
      ...
      dchr_no_nuova_real_aree_siti: [null, Validators.required],
      dchr_spec_cond_amb_screening: new FormControl(
  ...
  /**
   * Funzione per validare formTarget e abilitare i campi di input in base al valore del formTarget
   * @param formTarget 
   * @param elem1 
   * @param elem2 
   */
   validaEAbilitaCampi(formTarget, elem1, elem2?: any) {
    // Imposta il validatore per il campo principale
    this.setValid(formTarget);
    // Abilita i campi solo se il checkbox è selezionato
    //this.abilitaCampi(formTarget, elem1, elem2);
  }

  /**
   * Funzione che abilita o disabilita i campi di input in base al valore del formTarget
   * @param formTarget Evento del form che contiene l'id del campo
   * @param elem1 Nome del campo da abilitare/disabilitare
   * @param elem2 Nome del secondo campo da abilitare/disabilitare (opzionale)
   */
  abilitaCampi(formTarget, elem1, elem2?: any) {
    const indice = formTarget.currentTarget.id;
    let flagAbilita = this.formDichiarazioni.get(indice).value;

    if (flagAbilita) {
      // Se il checkbox è selezionato, abilita l'input di testo
      this.formDichiarazioni.get(elem1).enable();
      this.formDichiarazioni.get(elem1).setValidators(Validators.required);
      this.formDichiarazioni.get(elem1).updateValueAndValidity();
      this.formDichiarazioni.get(elem1).setValue(null);
      if (elem2) {
        this.formDichiarazioni.get(elem2).enable();
        this.formDichiarazioni.get(elem2).setValidators(Validators.required);
        this.formDichiarazioni.get(elem2).updateValueAndValidity();
        this.formDichiarazioni.get(elem2).setValue(null);
      }
    } else {
      // Se il checkbox non è selezionato, disabilita l'input di testo
      this.formDichiarazioni.get(elem1).setValue(null);
      this.formDichiarazioni.get(elem1).disable();
      this.formDichiarazioni.get(elem1).setValidators(null);
      this.formDichiarazioni.get(elem1).updateValueAndValidity();
      this.formDichiarazioni.get(elem1).setValue(null);
      if (elem2) {
        this.formDichiarazioni.get(elem2).disable();
        this.formDichiarazioni.get(elem1).setValidators(null);
        this.formDichiarazioni.get(elem2).updateValueAndValidity();
        this.formDichiarazioni.get(elem2).setValue(null);
      }
  ...
  getValueCodOfSelectedRadioButtonSpecCondAmbScreening(form): {
    cod: string;
    des: string;
  } {
    if (form.get('dchr_spec_cond_amb_screening').value === 'Si') {
      return {
        cod: 'dchr_spec_cond_amb_screening_si',
        des: this.DVER_C.DICHIARAZIONE_SPEC_COND_AMB_SCREENING_TRUE,
      };
    } else if (form.get('dchr_spec_cond_amb_screening').value === 'No') {
      return {
        cod: 'dchr_spec_cond_amb_screening_no',
        des: this.DVER_C.DICHIARAZIONE_SPEC_COND_AMB_SCREENING_FALSE,
      };
    } else {
      return null; // Nessun radio button selezionato
    }
  ...
  private caricaQuadro() {
    ...
    const arDichiarazioni = this.setArDichiarazioni();
    const QDR_DICHIARAZIONE = this.setQdr_dichiarazioni();

    //SCRIVA-1660
    if (this.adempimento.id_adempimento === 1) {
      const dichiarazioneNoRealAreeSiti = this.dichiarazioneNoRealAreeSiti();
      const dichiarazioneSpecCondAmbScreening =
        this.dichiarazioneSpecCondAmbScreening();
      const positionDichNoRealAreeSiti = 3;
      const positionDIchSpecCondAmbScreening = 4;
      arDichiarazioni.splice(
        positionDichNoRealAreeSiti,
        0,
        dichiarazioneNoRealAreeSiti
      );
      arDichiarazioni.splice(
        positionDIchSpecCondAmbScreening,
        0,
        dichiarazioneSpecCondAmbScreening
      );

      // Aggiungo le dichiarazioni al QDR_DICHIARAZIONE
      QDR_DICHIARAZIONE['dchr_no_nuova_real_aree_siti'] = {
        check: !!v.get('dchr_no_nuova_real_aree_siti').value,
      };
      QDR_DICHIARAZIONE['dchr_spec_cond_amb_screening'] = {
        radio: v.get('dchr_spec_cond_amb_screening').value,
      };
    }
    ...
    //     des_dichiarazione: `${this.DVER_C.dichiarazioniVIAACSpecifica(
    //       webLink2,
  ...
  /**
   * Funzione che crea l'array da utilizzare per le dichiarazioni.
   * @returns
   */
  private setArDichiarazioni() {
    const v: FormGroup = this.formDichiarazioni;
    ...
    const datiACRegionePiemonte: IDatiACRegionePiemonteVER = {
      webLink: this.acWebPiemonte,
    };
    const datiACSpecifica: IDatiACSpecificaVER = {
    ...
    const arDichiarazioni = [
...

      {
        ...
        des_dichiarazione:
          //"dichiara di essere consapevole che l'Amministrazione pubblicherà la documentazione trasmessa con la presente istanza (d.lgs.152/2006, art. 19, comma 3) sul sito web istituzionale Valutazioni ambientali: http://www.sistemapiemonte.it/cms/privati/ambiente-e-energia/servizi/540-valutazioni-ambientali",
          this.DVER_C.dichiarazioneACPubVER.descrizioneDichiarazione(
            this.paramsDichiarazioneACPubVER,
            this.cod_competenza_territorio
          ),
      },
      {
      ...
      //   des_dichiarazione: `${this.DVER_C.dichiarazioniVIARegionePiemonte(
      //     webLink1
      //   )}`,
    ...
    return arDichiarazioni;
  }

  /**
   * Funzione che crea l'oggetto QDR_DICHIARAZIONE da utilizzare nel riepilogo e quadro dichiarazioni.
   * @returns
   */
  private setQdr_dichiarazioni() {
    const v: FormGroup = this.formDichiarazioni;
    ...
    return QDR_DICHIARAZIONE;
  }

  private dichiarazioneNoRealAreeSiti(): any {
    const form: FormGroup = this.formDichiarazioni;
    const check: boolean = !!form.get('dchr_no_nuova_real_aree_siti').value;
    const obbligatorio: boolean = true;

    const cod_dichiarazione: string = 'dchr_no_nuova_real_aree_siti';
    const des_dichiarazione: string =
      this.DVER_C.DICHIARAZIONE_NO_NUOVA_REAL_AREE_SITI_DESCR;

    // Creo l'oggetto con le informazioni per la dichiarazione della nuova realizzazione aree siti
    const dichiarazione = {
      check,
      obbligatorio,
      cod_dichiarazione,
  ...
  private dichiarazioneSpecCondAmbScreening(): any {
    const form: FormGroup = this.formDichiarazioni;
    const check: boolean = form.get('dchr_spec_cond_amb_screening').value;
    const obbligatorio: boolean = true;
    const specCondAmbScreening =
      this.getValueCodOfSelectedRadioButtonSpecCondAmbScreening(form);
    const cod_dichiarazione: string = specCondAmbScreening.cod;
    const des_dichiarazione: string = specCondAmbScreening.des;

    // Creo l'oggetto con le informazioni per la dichiarazione della spec cond amb screening
    const dichiarazione = {
      check,
      obbligatorio,
      cod_dichiarazione,
  ...
  private dichiarazioneDatiPersonaliGDPR(
    datiACRegionePiemonte: IDatiACRegionePiemonteVER,
    datiACSpecifica: IDatiACSpecificaVER
  ): any {
    ...
    if (this.isACPrincipaleRegionePiemonte) {
      ...
      des_dichiarazione = this.DVER_C.dichiarazioniVIARegionePiemonte(linkACRP);
      // #
    } else {
  ...
  salvaDatiQuadro() {
    ...
    this.saveDataQuadro(configsBuild).subscribe(
      (res) => {
        this.spinner.hide();
        this.saveWithPut = true;
        this.setStepCompletedEmit('DichiarazioniVerComponent', true);
  ...
  protected isStepValid():Observable<boolean> {
    if (!this.formDichiarazioni.valid) {
      ...
      // Form NON valido
      this.formDichiarazioni.get('dchr_doc_segr_ind_com').markAsTouched();
      this.formDichiarazioni.get('dchr_doc_amm_nopubb').markAsTouched();
      this.formDichiarazioni.get('dchr_pagam_bollo').markAsTouched();
      this.formDichiarazioni.get('dchr_pagam_oneri_istruttori').markAsTouched();
      this.formDichiarazioni.get('dchr_spec_cond_amb_screening').markAsTouched();
      return of(false);
    } else {
      return of(true);
    }
   ...
   * @returns IDichiarazioneACPubVERSpecifica con i parametri.
   */
  get paramsDichiarazioneACPubVER(): IDichiarazioneACPubVERSpecifica {
    // Creo l'oggetto con i paramtri
    const params: IDichiarazioneACPubVERSpecifica = {
