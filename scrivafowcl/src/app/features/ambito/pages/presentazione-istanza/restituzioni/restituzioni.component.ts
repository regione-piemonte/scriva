...
import { RestituzioniOperaModalComponent } from './modals/restituzioni-opera-modal/restituzioni-opera-modal.component';
import { RestituzioniConsts } from './utilities/restituzioni.consts';
import { PresentazioneIstanzaService } from '../../../services/presentazione-istanza/presentazione-istanza.service';

@Component({
  selector: 'app-restituzioni',
  templateUrl: './restituzioni.component.html',
  styleUrls: ['../opere/opere.component.scss'],
  providers: [OpereVerificheStepService],
})
export class RestituzioniComponent
  extends OpereComponent
  implements OnInit, OnDestroy, AfterViewInit
{
  /** RestituzioniConsts con le informazioni costanti per il componente. */
  public OPERE_CONSTS = new RestituzioniConsts();

  /** string costante che definisce il contenitore per agganciare i messaggi di segnalazione. */
  readonly ALERT_TARGET_MODALE: string = 'restituzioni-content';

  /**
   * string che definisce il nome del componente di riferimento.
   * @override
   */
  protected componentName: string = 'RestituzioniComponent';

  /**
   * any per gestire il componente da usare per la modale di dettaglio delle opere.
   * @override
   */
  protected componenteModale: any = RestituzioniOperaModalComponent;
