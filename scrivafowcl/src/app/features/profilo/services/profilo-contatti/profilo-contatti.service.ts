...
export class ProfiloContattiService {
  ...
  generateControlsConfig(): DynamicObjAny {
    ...
    // # Email
    controlsConfig[this.P_C.FC_EMAIL] = new FormControl(
      { value: null, disabled: false },
      { validators: [] }
    );
    // # Telefono
    controlsConfig[this.P_C.FC_TELEFONO] = new FormControl(
      { value: null, disabled: false },
      { validators: [] }
    );
  ...
  setFormConDatiProfilo(
    ...
    const keyTel = this.P_C.FC_TELEFONO;
    // Recupero dall'oggetto le informazioni
    const email = datiProfilo?.email ?? '';
    const telefono = datiProfilo?.telefono ?? '';
    ...
    f.get(keyEmail).setValue(email, o);
    f.get(keyTel).setValue(telefono, o);
  }
