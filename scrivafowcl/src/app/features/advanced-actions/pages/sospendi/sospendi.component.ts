...
@Component({
  selector: 'app-sospendi',
  templateUrl: './sospendi.component.html',
  styleUrls: ['./sospendi.component.scss'],
})
export class SospendiComponent
  extends AbstractAdvancedAction
  implements OnInit
{
  chiave: AdvancedActionsChiavi = AdvancedActionsChiavi.SOSPENDI;
  codMaschera = AdvancedActionsMaschere.SOSPENDI;
  ...
  saveIstanza() {
    super
      ...
      .subscribe({
        next: (res: any) => {
          this._spinner.hide();
          super.generaEvento(this.tipoEventoEnum.SOSP_PROC, 'P021');
        },
        error: (e: any) => {
